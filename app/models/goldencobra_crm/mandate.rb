...
module GoldencobraCrm
  class Mandate < ActiveRecord::Base
    ...
    def generate_external_id
      last_used_number = GoldencobraCrm::Mandate.order(:created_at).last.internal_id.to_i + 1 + Goldencobra::Setting.for_key('goldencobra_crm.api.mandate.initial_counter').to_i
      self.internal_id = last_used_number
      formated_number = last_used_number.ljust(Goldencobra::Setting.for_key('goldencobra_crm.api.mandate.length').to_i, "0")
